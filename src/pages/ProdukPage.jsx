    ...
    <div>
      ...
      <div className={styles.card} style={{ padding: 0, overflow: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
              {["Foto", "Nama", "Kategori", "Satuan", "Harga", "Stok", "Min", "Supplier", "Aksi"].map((h) => (
                  ...
                  <td className={styles.td}>
                    <div style={{ display: "flex", gap: 5 }}>
                      <button className={`${styles.btn} ${styles.btnoutline}`} style={{ padding: "4px 10px", fontSize: 11 }}
                        onClick={() => {
                          setProdForm({ name: p.name, category: p.category, unit: p.unit, price: String(p.price), stock: String(p.stock), min_stock: String(p.min_stock), supplier_id: String(p.supplier_id || "") });
                          setProdImage(null);
                          setProdModal(p);
                        }}>Edit</button>
