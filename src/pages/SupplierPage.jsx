    ...
    <div>
      <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <input id="supplier-search" name="supplier-search" className={`${styles.inp} max-w-[220px]`}
            placeholder="🔍 Cari supplier..." value={search} onChange={(e) => setSearch(e.target.value)} />
          <span className="text-[13px] text-gray-500">{filtered.length} supplier</span>
        </div>
            ...
            <div key={s.id} className={`${styles.card} p-4`}>
              <div className="flex justify-between items-start mb-2.5">
                <div className="flex-1">
                  <div className="font-extrabold text-sm text-bbs-green-dark ">{s.name}</div>
                  <div className="text-[11px] text-gray-400 mt-0.5">👤 {s.contact}</div>
                </div>
                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold shrink-0 ${s.status === "Aktif" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-600"}`}>
                  {s.status}
                </span>
              </div>
              ...
              <div className="flex gap-1.5 border-t border-[#f0f5f0]  pt-2.5">
                <button className="flex-1 py-1 text-[11px] font-bold rounded-lg border border-bbs-green text-bbs-green bg-transparent cursor-pointer"
